...
/**
 * <p>
 * A mathematical program, more precisely a linear program or a mixed integer program, consisting of variables (with
 * bounds), constraints and zero or one objective function. Such a mathematical program defines a set of feasible
 * solutions. A feasible solution is an association of a value to each of the variables of the program, satisfying all
 * the constraints in the program. The set of feasible solutions is also called the feasible region. This object may
 * also represent programs that have an empty feasible region, or equivalently, no feasible solutions, or equivalently,
 * that are infeasible. Some of the feasible solutions are said to be optimal, these are the solutions that have the
 * best value according to the objective function associated with the program. This class uses the name <em>Problem</em>
 * rather than Program because of the (even bigger) ambiguity of the latter word.
 * </p>
 * <p>
 * A variable bounds may be set to anything, as long as the lower bound is lower than or equal to the upper bound,
 * independently of the variable type. For example, a boolean typed variable may have a lower bound of -3 and upper
 * bound of 0.8. When solving the problem, the variable will be considered as having the most restrictive bounds imposed
 * by either its bounds or its type. In the example, the variable would be constrained to zero.
 * </p>
 * <p>
 ...
 * {@link BigDecimal}, {@link BigInteger} will pose no problem. Using other types of numbers is unsupported.
 * </p>
 * <p>
 * Some implementations of this interface may be read-only (either because they are immutable or because they are a
 * read-only view), in which case an attempt to change the state of this object will throw
 * {@link UnsupportedOperationException}.
 * </p>
 * <p>
 * Two such problems are considered equal when they define the same variables (as per {@link #equals}), constraints,
 * objective function, and bounds for the variables as determined by the {@link Number#doubleValue()} method. The
 * equality between two problems does not take the names into account: neither the problem, variables or constraints
 * names are considered. Two equal problems, as determined by this class {@link #equals(Object)} method, have the same
 * set of feasible solutions, although a non equality between two problems does <em>not</em> imply that they have
 * different sets of feasible solutions.
 * </p>
 * 
 * @param <V>
...
public interface LpProblem<V> {
    ...
    public Function<Object, String> TO_STRING_NAMER = Functions.toStringFunction();

    /**
     * Two problems are considered equal when they define the same variables (as per {@link #equals}), constraints,
     * objective function, and bounds for the variables as determined by the {@link Number#doubleValue()} method. The
     * equality between two problems does not take the names into account: neither the problem, variables or constraints
    ...
    /**
     * Adds the variable to this problem if it is not already in, with a default type of REAL, no name, a lower bound
     * equal to zero and a positive infinite upper bound.
     * 
     * @param variable
     *            not <code>null</code>.
    ...
    /**
     * Removes all the variables and constraints, objective function, name, namers set in this problem. As a result of
     * this call, this problem has the same visible state as a newly created, empty problem.
     */
    public void clear();
    ...
    /**
     * Retrieves the lower bound of the given variable. The lower bound is less or equal to the upper bound.
     * 
     * @param variable
    ...
    /**
     * Retrieves the name of the variable as given by the variable namer function and transformed to empty string if the
     * variable namer function gave a <code>null</code> answer.
     * 
     * @param variable
     *            not <code>null</code>, must be a variable of this problem.
     ...
     * the namer function is set back to the default function. The function is never given a <code>null</code>
     * constraint; however the constraint id may be <code>null</code>.
     * </p>
     * 
     * @param namer
     ...
     * </p>
     * <p>
     * The default namer uses the constraint id transformed to string as per {@link LpConstraint#getId()} and
     * {@link #toString()}, and the empty string if the constraint has a <code>null</code> id.
     * </p>
     * 
     * @return not <code>null</code>.
    ...
    /**
     * Retrieves the upper bound of the given variable. The upper bound is greater than or equal to the lower bound.
     * 
     * @param variable
     *            not <code>null</code>, must be a variable of this problem.
    ...
    /**
     * <p>
     * Sets the lower and upper bounds of a variable. Adds the variable to this problem if it is not already in, with a
     * REAL type as default type. The lower bound must be less than or equal to the upper bound.
     * </p>
     * <p>
     * The variable type does <em>not</em> impose constraints on the acceptable bounds that may be associated to it.
     * </p>
     * 
     * @param variable
     *            not <code>null</code>.
     * @param lowerBound
     *            <code>null</code>, or minus infinity, for a lower bound equal to minus infinity, may not be positive
     *            infinity.
     * @param upperBound
     *            <code>null</code>, or positive infinity, for an infinite upper bound, may not be negative infinity.
     * @return <code>true</code> iff the call modified the state of this object, or equivalently, iff the variable did
     *         not exist previously in this object or at least one of the given bounds is different than the previous
     *         bound. Two bounds are considered equal iff they are both infinite with the same sign or they would be
     *         considered equal according to the given bound {@link #equals(Object)} method. For example, when given as
     *         new bound a {@link BigDecimal} with the same value as the current bound but a different scale, this
     *         method will change the bound and return <code>true</code>, because such numbers are not considered equal
     *         as per {@link BigDecimal#equals}.
     */
    public boolean setVariableBounds(V variable, Number lowerBound, Number upperBound);

    /**
     * Sets or removes the type of a variable. Adds the variable to this problem if it is not already in, with a default
     ...
     * 
     * @return not <code>null</code>, by default returns the {@link #TO_STRING_NAMER} function.
     */

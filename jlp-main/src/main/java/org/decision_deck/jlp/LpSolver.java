 ...
 * {@link #writeProblem} close the underlying solver after the job is done, thus it is not necessary to call close again
 * after these calls. After a call to {@link #solve()} or {@link #writeProblem}, the underlying solver pointer is not
 * valid any more.
 * 
 ...
 * {@link LpResultStatus#FEASIBLE} status if no objective function is given.
 * </p>
 * <p>
 * Note that solvers may give a solution that is slightly outside the defined bounds for the variable.
 * </p>
 * 
 * @param <V>
...
public interface LpSolver<V> {
    ...
    /**
     * In manual close mode, must be used after use in order to release possibly acquired resources after a call that
     * initialized the underlying solver. In auto close mode, it is not necessary to call this method. If there is
     * nothing to close (e.g. because everything is closed already or no resources have been acquired yet), this method
     * has no effect.
     * 
    ...
    /**
     * Retrieves a writable view that allow to query and set the values of the parameters of this solver instance.
     * 
    ...
    /**
     * Indicates if this solver has a result object available, or equivalently, if a solve has been attempted. The
     * method {@link #getResult()} may be called iff this method returns <code>true</code>.
     * 
    ...
    /**
     * Retrieves the result of solving the bound problem (the last time it was solved). This method may not be called
     * before a solve has been attempted. If a solve has been attempted, this method necessarily yields a valid result
    ...
    /**
     * Switches to manual close mode, initializes the underlying solver and returns it. This is a writeable view to the
     * solver underlying this object, thus modifications from this object are reflected to the returned solver, and
     * conversely. Modifying the underlying solver through the returned object may make this object behave in an
     * unpredictable manner, as it does not necessarily have a means to detect such changes. Therefore, the user must
     * choose between two possible uses: either only use the underlying solver in read mode, not changing its state, or
     * change its state in which case this object should not be used any more afterwards. Only the close method of this
    ...
    /**
     * Copies the given problem data (including its name) into the problem bound to this solver. Any information
     * possibly existing in this object problem is lost. This solver keeps a reference to the given problem, no
     * defensive copy is done. Changes to the given problem will change the problem in this solver, and conversely. To
     * obtain a defensive copy, use {@link LpProblems#copyTo(LpProblem, LpProblem)} with {@link #getProblem()}. This
     * should be preferred because the latter method allows this object to use any implementation it considers most
     * suitable for optimisation (in particular to track changes to the problem, useful in manual close mode). Using
     * this method may prevent this object from applying certain optimisations in manual close mode when the problem is
    ...
    /**
     * <p>
     * Turns this object into auto close or manual close mode. When this object auto closes (the default), the
     * underlying solver is initialized when needed and closed immediately afterwards, before the call that triggered
     * the initialization ends. This permits to shield the user from having to think about closing the object after use.
     * However, it renders certain optimizations impossible, such as not having to rebuild the entire problem each time
     * a solve is asked. Turning the auto close off switches to manual close mode, thus makes this object never close
     * the underlying solver automatically, which implies the user has to call {@link #close()} after use.
     * </p>
     * <p>
     * Turning the auto close on switches to auto close mode and immediately close the underlying solver if it was
     * intialized.
     * </p>
     * 
     * TODO implement this.
     * 
     * @param autoClose
     *            <code>true</code> for auto close mode, <code>false</code> to enable manual close mode.
     */
     ...
     * 
     * @param format
     *            <code>null</code> is equivalent to {@link LpFileFormat#SOLVER_PREFERRED}.
     * @param destination
     *            not <code>null</code>.
     * @throws LpSolverException
     *             if a solver-specific exception occurs, or the solver does not implement writing problem files, or

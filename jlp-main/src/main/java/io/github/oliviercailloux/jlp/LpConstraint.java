 ...
 * </p>
 * <p>
 * Such a constraint may have a id that may be used to describe or identify it,
 * e.g. its string form may be used when writing a mathematical problem to a
 * file. Although this object will accept any id, when dealing with several
 * constraints, it is strongly recommanded to either use <code>null</code> ids
 * (thus to not use ids) or to use unique ids each identifying with no ambiguity
 * one constraint, as objects using these constraints might rely on their ids
 * being truly unique identifiers. Thus, it is suggested to ensure that the
 * following relation holds on a set of constraints C: for any two constraints
 * c1, c2 in C whose ids are both non <code>null</code>, c1 id must
...
public class LpConstraint<V> {
	...
	/**
	 * Retrieves a string identifier for this constraint, if possible. This is the
	 * string form of this constraint id, as per {@link #toString()}, or the empty

...
public class ConstraintsUtils {
	static public void assertSatisfied(double lhs, ComparisonOperator op, double rhs) {
		if (!satisfied(lhs, op, rhs)) {
			throw new IllegalStateException("Unsatisfied: " + lhs + op + rhs + ".");
		}
	}

	static public void assertSatisfied(int lhs, ComparisonOperator op, int rhs) {
		if (!satisfied(lhs, op, rhs)) {
			throw new IllegalStateException("Unsatisfied: " + lhs + op + rhs + ".");
		}
	}
	 ...
	 *         the given operator.
	 */
	static public boolean satisfied(double lhs, ComparisonOperator op, double rhs) {
		checkNotNull(op);
		switch (op) {
		case EQ:
			return lhs == rhs;
	...
	static public boolean satisfied(int lhs, ComparisonOperator op, int rhs) {
		checkNotNull(op);
		switch (op) {
		case EQ:
			return lhs == rhs;

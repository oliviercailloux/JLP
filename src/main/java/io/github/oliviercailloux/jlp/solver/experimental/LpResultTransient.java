...
import io.github.oliviercailloux.jlp.result.LpResultImpl;
...
public interface LpResultTransient<V> {
	...
	/**
	 * <p>
	 * Retrieves one solution found to the problem solved, or <code>null</code> if
	 * no solution was saved and it is not available any more. If the result of the
	 ...
	 * </p>
	 * <p>
	 * The returned object may become invalid (exactly when this method returns
	 * <code>null</code>), in which case querying it again will trigger an error.
	 * Apart from that, it is immutable. When this method returns <code>null</code>,
	 * hence when a previously returned object becomes invalid, depends on how this
	 * result object got obtained. See documentation in the object which served to
	 * get these results. TODO say this in front of this class, also serves for
	 * parameters. Say that LpResult is simpler (never gets invalid).
	 * </p>
	 * 
	 * @return <code>null</code> if no solution was saved and it is not available
	 *         any more.
	 * @see #getResultStatus()
	...
	/**
	 * Retrieves the parameters that have been used to obtain this result, or
	 * <code>null</code> if the parameters have not been saved and are not available
	 * anymore. TODO should be immutable.
	 * 
	 * @return <code>null</code> if the parameters have not been saved and are not
	 *         available anymore.
	 */
